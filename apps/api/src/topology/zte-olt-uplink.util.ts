...
export function extractInterfaceBlock(
  ...
  const start = lines.findIndex((l) =>
    new RegExp(`^\\s*interface\\s+${escapeRe(ifName)}\\s*$`, 'i').test(l),
  )
  if (start < 0) return ''
  const out: string[] = []
  for (let i = start + 1; i < lines.length; i++) {
    if (/^\s*interface\s+/i.test(lines[i]) || /^\s*!\s*$/.test(lines[i])) break
...
export function parseUplinkConfigBlock(block: string): {
  ...
  for (const raw of block.split(/\r?\n/)) {
    ...
    const tagM = line.match(
      /^switchport\s+vlan\s+(\d+(?:[,-]\d+)*)\s+tag\b/i,
    )
    if (tagM) {
      for (const v of expandVlanList(tagM[1])) tagged.add(v)
    }
    const untagM = line.match(
      /^switchport\s+vlan\s+(\d+)\s+untag\b/i,
    )
...
export function parseInterfaceStatus(text: string): {
  ...
  if (/line protocol is up|is up,/i.test(text) && !/administratively down/i.test(text)) {
    const speed =
    ...
    if (speed) {
      ...
      negotiation = status.includes('Full') || status.includes('Half')
        ? `Forced ${status}`
        : 'Auto'
    } else {
      status = 'Up'
  ...
  if (auto) negotiation = negotiation?.startsWith('Forced')
    ? negotiation
    : 'Auto'

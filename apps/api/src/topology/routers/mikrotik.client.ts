...
export interface MikroTikProbeResult {
  ...
  /** Contadores del puerto de salida a Internet (si se pidió). */
...
export class MikrotikClient {
  ...
  async probe(params: {
    ...
    password: string;
    protocol?: string;
    /** Nombre RouterOS del puerto/VLAN de salida a Internet (rx/tx). */
    egressInterfaceName?: string | null;
  ...
  private async probeApi(params: {
    ...
    try {
      ...
      const health = await client
        .print('/system/health')
        .catch(() => [] as Record<string, string>[]);
      const allIfaces = params.egressInterfaceName
        ? await client
            .print('/interface')
            .catch(() => [] as Record<string, string>[])
        : [];
      ...
      const egress = this.pickEgressCounters(
        allIfaces,
        params.egressInterfaceName,
      );
  ...
  private async probeRest(params: {
    ...
    const [
      ...
      params.egressInterfaceName
        ? this.httpsGetJson(`${base}/interface`, auth).catch(() => [])
        : Promise.resolve([]),
    ]);
  ...
  private pickEgressCounters(
    ifaces: Record<string, string>[],
    name?: string | null,
  ): Pick<
    MikroTikProbeResult,
    'egressInterfaceName' | 'egressRxBytes' | 'egressTxBytes'
  > {
    const want = name?.trim();
    if (!want || !ifaces.length) return {};
    const row =
      ifaces.find((r) => r.name === want) ??
      ifaces.find((r) => (r.name || '').toLowerCase() === want.toLowerCase());
    if (!row) return { egressInterfaceName: want };
    const rx = Number(row['rx-byte'] ?? row['rx-bytes']);
    const tx = Number(row['tx-byte'] ?? row['tx-bytes']);
    return {
      egressInterfaceName: row.name || want,
      egressRxBytes: Number.isFinite(rx) ? rx : undefined,
      egressTxBytes: Number.isFinite(tx) ? tx : undefined,
    };
  }
    ...
    try {
      return await this.withDeviceLock(params.host, params.port, async () => {
        ...
        try {
          ...
          if (existing?.['.id']) {
            if (vlanFiltering && !this.isTruthy(existing['vlan-filtering'])) {
              await client.write([
  ...
  async setBridgePort(params: {
    ...
    try {
      return await this.withDeviceLock(params.host, params.port, async () => {
        const protocol = params.protocol ?? 'api_ssl';
        const pvid =
          params.pvid != null && Number.isFinite(params.pvid) ? params.pvid : 1;
      ...
      return await this.withDeviceLock(params.host, params.port, async () => {
        ...
        if (protocol === 'rest_https') {
          ...
          const existing = rows.find((v) => {
            const ids = (v['vlan-ids'] || '').split(/[,\s-]+/).map(Number);

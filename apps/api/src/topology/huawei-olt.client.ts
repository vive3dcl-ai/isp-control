...
export type HuaweiConnectedOnu = ZteConnectedOnu;
export type HuaweiConnectedOnuDetail = ZteConnectedOnuDetail;
...
export class HuaweiOltClient {
  ...
  async probe(
    ...
    try {
      return await this.run(params, false, async (io) => {
        ...
        const cards = parseHuaweiBoard(board) as ZteOltCard[];
  ...
  async listCards(params: CliParams): Promise<ZteOltCardsResult> {
  ...
  async getConnectedOnuDetail(params: CliParams & { onuIf: string }) {
    ...
    try {
      const onu = await this.run(params, false, async (io) => {
        ...
        )[0] as ZteConnectedOnu | undefined;

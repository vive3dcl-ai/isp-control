/** Parsers for per-ONU inventory from ZTE C3xx CLI (read-only shows). */
...
export function parseOnuBaseInfo(
  ...
  for (const raw of text.split(/\r?\n/)) {
    ...
    const full = line.match(/^((?:gpon|epon)-onu_[\d/]+:\d+)\s+(.*)$/i)
    if (full) {
      onuIf = full[1]
...
export function parseOnuRxByIf(
  ...
  for (const raw of text.split(/\r?\n/)) {
    ...
    const withIf = line.match(
      /((?:gpon|epon)-onu_[\d/]+:\d+)\s+(-?\d+(?:\.\d+)?)\s*(?:\(?\s*dbm)?/i,
    )
    if (withIf) {
...
export function parseOnuDetailInfo(text: string): ZteOnuDetailInfoParsed {
  ...
  for (const m of text.matchAll(labelRe)) {
    fields.push({ label: m[1].replace(/\s+/g, ' ').trim(), value: m[2].trim() })
  }
...
export function parseRemoteOnuEquip(text: string): ZteRemoteOnuEquip {
  ...
  const fields: Array<{ label: string; value: string }> = []
  for (const m of text.matchAll(/^\s*([A-Za-z][A-Za-z0-9 /_-]*)\s*[:=]\s*(.+)$/gm)) {
...
export function formatOnuStatusReport(parts: {
  ...
  const usable = (raw?: string | null) => {
    const t = raw?.trim() || ''
    if (!t) return ''
    if (/%Error\s*\d+|Invalid\s+(?:input|parameter|command)/i.test(t)) return ''
    return t
  }
  ...
  if (parts.detail?.history?.length) {
    out.push(
      `     ${pad('Authpass Time', 22)}${pad('OfflineTime', 24)}Cause`,
    )
...
export function parseOnuInterfacesFromRunningConfig(
  ...
  const re =
    /^interface\s+((?:gpon|epon)-onu_[\d/]+:\d+)\s*$/gim
...
export function oltIfFromOnuIf(onuIf: string): string | null {
  const m = onuIf.match(/^((?:gpon|epon)-onu_([\d/]+)):(\d+)$/i)
  if (!m) return null
  return m[1].replace(/-onu_/i, '-olt_')
}
...
export function parseOnuInterfaceRates(text: string): {
  ...
  return {
    ...
    downloadBps:
      downloadBps != null && Number.isFinite(downloadBps)
        ? downloadBps
        : null,
    uploadPps:
      uploadPps != null && Number.isFinite(uploadPps) ? uploadPps : null,
    downloadPps:
      downloadPps != null && Number.isFinite(downloadPps)
        ? downloadPps
        : null,
  }
}
...
export function parseOnuUncfg(
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    const m = trimmed.match(
      /^((?:gpon|epon)-onu_[\d/]+(?::\d+)?)\s+([A-Za-z0-9]{8,16})\s*(\S+)?/i,
    )
    if (!m) {
      // SN-only style: ZTEG00000002 unknown
      const snOnly = trimmed.match(
        /^([A-Za-z0-9]{10,16})\s+(\S+)?$/,
      )
      if (snOnly && defaultOltIf) {
        ...
        const parts = defaultOltIf.match(
          /^(?:gpon|epon)-olt_(\d+)\/(\d+)\/(\d+)$/i,
        )
        rows.push({
          oltIf: defaultOltIf,
          onuIfHint: null,
    ...
    const oltIf =
      defaultOltIf ||
      onuIfHint.replace(/-onu_/i, '-olt_').replace(/:\d+$/, '')
    const parts = oltIf.match(/^(?:gpon|epon)-olt_(\d+)\/(\d+)\/(\d+)$/i)
...
/** Build gpon-onu_…:id from gpon-olt_… + onuId */
export function onuIfFromOltIf(oltIf: string, onuId: string | number): string {
  return `${oltIf.replace(/-olt_/i, '-onu_')}:${onuId}`
}

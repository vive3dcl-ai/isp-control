...
import type { OltInventoryCache } from '../../olts/olt-inventory-cache';
...
export class NetworkDevice {
  ...
  /**
   * Puerto físico de salida a Internet (nombre RouterOS, p.ej. ether1).
   * Usado para gráficos de tráfico en el dashboard.
   */
  @Column({
    name: 'internet_egress_port_name',
    type: 'varchar',
    length: 80,
    nullable: true,
  })
  ...
  /** VLAN opcional sobre el puerto de salida (null = interfaz física / native). */
  @Column({
    name: 'internet_egress_vlan_id',
    type: 'int',
    nullable: true,
  })

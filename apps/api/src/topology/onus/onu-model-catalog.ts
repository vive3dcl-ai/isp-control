...
/**
 * Modelo usable para persistir / mostrar. Null si vacío o placeholder (N/A).
 */
export function usableOnuModelName(
  raw: string | null | undefined,
): string | null {
  const n = normalizeOnuModelName(raw ?? '');
  if (!n || isPlaceholderOnuModel(n)) return null;
  return n;
}

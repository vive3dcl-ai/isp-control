...
export class OnuCatalogAdminService {
  ...
  /**
   * When an ONU model is seen on an OLT / SW info / ACS:
  ...
  async ensureModelSeen(
    ...
    const vendorOverride = opts?.vendor?.trim().toLowerCase();
    const vendor =
      vendorOverride === 'huawei' ||
      vendorOverride === 'zte' ||
      vendorOverride === 'fiberhome'
        ? vendorOverride
        : inferOnuVendor(name);
    ...
    if (!item) {
      ...
      item = await this.catalogRepo.save(
        this.catalogRepo.create({
          ...
          note: 'Detectado automáticamente — pendiente de registro',
          isActive: true,
          registrationStatus: 'pending',
        }),
      );
    } else {
      if (normalizeOnuModelName(item.name) !== item.name) {
        item.name = normalizeOnuModelName(item.name);
      }
      // Corregir vendor si el SN dice FiberHome/ZTE/Huawei y el catálogo mentía
      // (HG* → inferOnuVendor=huawei aunque sea FiberHome).
      if (
        (vendor === 'fiberhome' || vendor === 'zte' || vendor === 'huawei') &&
        item.vendor !== vendor &&
        item.registrationStatus === 'pending'
      ) {
        item.vendor = vendor;
        item.imageKey = imageKeyForVendorCapability(vendor, item.capability);
      }
    ...
    if (syncToTenant) {
      ...
      // Alinear vendor del tipo tenant con el SN cuando hace falta.
      if (vendor === 'fiberhome' || vendor === 'zte' || vendor === 'huawei') {
        const typeRepo =
          await this.tenantConnections.getOnuTypeRepository(schemaName);
        const row = await typeRepo
          .createQueryBuilder('t')
          .where('LOWER(t.name) = LOWER(:name)', { name })
          .getOne();
        if (row && row.vendor !== vendor) {
          row.vendor = vendor;
          await typeRepo.save(row);
        }
      }
  ...
  /**
   * Cruza ONUs conectadas (SN) con GenieACS ProductClass y corrige `onu_type`
   * cuando el type OMCI de la OLT no coincide (p. ej. F600 vs HG6143D).
   * También asegura tipos listados para esos modelos reales.
   */
  /**
   * Una ONU: lee ProductClass/ModelName del ACS y actualiza `onu_type` si
   * conviene (placeholder o distinto del tipo OMCI de la OLT).
   * Resync / apply lo llaman **antes** de resolver el driver de parche.
   */
  async syncOnuTypeFromAcs(
    schemaName: string,
    onuId: string,
  ): Promise<{
    ok: boolean;
    ...
    await this.tenantConnections.ensureTenantSchema(schemaName);
    const onuRepo = await this.tenantConnections.getOnuRepository(schemaName);
    const onu = await onuRepo.findOne({
      where: { id: onuId },
      select: ['id', 'sn', 'onuType'],
    });
    if (!onu) {
      return {
        ok: false,
    ...
    const previous = onu.onuType?.trim() || null;
    ...
    if (!sn) {
      return {
        ...
        previous,
        model: null,
        message: 'Sin SN — no se puede consultar ACS',
      };
    }

    const acsMap = await loadAcsModelsBySerial([sn]);
    const acsModel = acsMap.get(sn.toUpperCase()) ?? null;
    if (!acsModel) {
      return {
        ok: true,
        changed: false,
        previous,
        model: null,
        message: 'ACS sin ProductClass para este SN',
      };
    }

    const snVendor = vendorFromSn(sn);
    const vendor = snVendor === 'other' ? null : snVendor;
    await this.ensureModelSeen(schemaName, acsModel, {
      syncToTenant: true,
      vendor,
    });

    if (!shouldApplyAcsModelToOnuType(onu.onuType, acsModel)) {
      return {
        ok: true,
        changed: false,
        previous,
        model: acsModel,
        message: `Modelo ACS «${acsModel}» (sin cambio)`,
      };
    }

    onu.onuType = acsModel;
    ...
    return {
      ...
      model: acsModel,
      message: previous
        ? `Modelo actualizado ${previous} → ${acsModel}`
        : `Modelo ACS «${acsModel}»`,
    };
  }

  async reconcileConnectedModelsFromAcs(schemaName: string): Promise<{
    scanned: number;
    acsHits: number;
    updated: number;
    models: string[];
    ...
    await this.tenantConnections.ensureTenantSchema(schemaName);
    const onuRepo = await this.tenantConnections.getOnuRepository(schemaName);
    const onus = await onuRepo.find({
      select: ['id', 'sn', 'onuType'],
    });
    const withSn = onus.filter((o) => !!o.sn?.trim());
    if (!withSn.length) {
      return { scanned: 0, acsHits: 0, updated: 0, models: [] };
    }

    const acsMap = await loadAcsModelsBySerial(withSn.map((o) => o.sn!));
    let updated = 0;
    const models = new Set<string>();

    for (const onu of withSn) {
      const sn = onu.sn!.trim().toUpperCase();
      const acsModel = acsMap.get(sn);
      if (!acsModel) continue;
      models.add(acsModel);

      const snVendor = vendorFromSn(sn);
      const vendor =
        snVendor === 'other' ? null : snVendor;

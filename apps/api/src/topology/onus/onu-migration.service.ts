...
export class OnuMigrationService {
  ...
  async scan(user: AuthUser, oltId: string, opts: { fromOlt?: boolean } = {}) {
    ...
    // Prefer ACS ProductClass over OLT OMCI type for display + later import.
    try {
      const sns = candidates
        .map((c) => c.sn?.trim())
        .filter((s): s is string => !!s);
      if (sns.length) {
        const acsMap = await loadAcsModelsBySerial(sns);
        for (const c of candidates) {
          const sn = c.sn?.trim().toUpperCase();
          if (!sn) continue;
          const acsModel = acsMap.get(sn);
          if (acsModel && shouldApplyAcsModelToOnuType(c.onuType, acsModel)) {
            c.onuType = acsModel;
          }
        }
      }
      ...
      this.logger.warn(
        `migration scan ACS models: ${err instanceof Error ? err.message : err}`,
      );
    }

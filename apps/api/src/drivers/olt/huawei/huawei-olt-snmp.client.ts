...
} from '../_shared/transport/snmp-health';
...
export type HuaweiSnmpOnuRow = SnmpOnuRow;
export type HuaweiSnmpMonitorResult = Omit<SnmpMonitorResult, 'source'> & {
...
export class HuaweiOltSnmpClient {
  ...
  async probeSnmp(params: HuaweiSnmpConn): Promise<SnmpProbeResult> {
    const community = params.snmpCommunity?.trim();
    if (!params.host?.trim() || !community) {
      return { ok: false, error: 'SNMP host/community missing' };
    }
  ...
  async walkOltPorts(params: HuaweiSnmpConn): Promise<SnmpPortsResult> {
    return this.withSnmpLock(params.host, 'background', () =>
  ...
  private async walkOltPortsUnlocked(
    ...
    try {
      ...
      const uplinks: SnmpPortRow[] = [];
      const ponByName = new Map<string, SnmpPortRow>();

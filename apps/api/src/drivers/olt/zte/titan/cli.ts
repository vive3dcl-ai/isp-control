...
/**
 * ZTE Titan (C6xx) OLT CLI driver (silo).
 * Family pinned to 'c6xx'. Shared parsers live in drivers/olt/zte/shared.
 */
@Injectable()
export class ZteTitanOltClient {
  private readonly logger = new Logger(ZteTitanOltClient.name);
  /**
   ...
   * C6xx: `onu N disable` on the OLT port; fallback `shutdown` on ONU if.
   */
   ...
   * C6xx: `onu N enable`. C3xx-style fallback: `no shutdown` on ONU if.
   */
  ...
  async applyOnuInternetTcont(params: {
    ...
    try {
      return await this.runConfigWrite(params, async (send, read) => {
        ...
        return { ok: true, message: `tcont 1 profile ${up}` };
      });
    } catch (err) {
      return {
        ok: false,

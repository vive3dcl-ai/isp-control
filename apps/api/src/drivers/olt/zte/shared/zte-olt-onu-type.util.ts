 ...
 *
 * `vendorFromSn` vive en `drivers/onu/infra` (puro); re-export aquí para
 * call sites OLT que ya importaban este módulo.
 */
...
export function parseOnuTypeList(
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (
      /^(Pon\s*Type|Onu\s*type|Type\s*name|----|show\s|Total)/i.test(trimmed)
    ) {
      continue;
    }

...
export function parseOnuBaseInfo(
  ...
  const family: 'gpon' | 'epon' = oltIfHint?.startsWith('epon')
    ? 'epon'
    : 'gpon';
  const isEmptyName = (t: string) => !t || /^(--|—|-|n\/a|na|none|null)$/i.test(t);
  ...
  for (const raw of text.split(/\r?\n/)) {
    ...
    if (snIdx > 0) {
      ...
      if (before.length >= 2) {
        ...
        const modelTok = before.find((t) => isModel(t));
        if (modelTok) onuType = modelTok;
        else {
          const last = before[before.length - 1];
          if (last && !isEmptyName(last)) onuType = last;
        }
...
export function parseRemoteOnuEquip(text: string): ZteRemoteOnuEquip {
  ...
  /** OLT pone N/A en hojas aún no listas (Product SN, Region, a veces Model). */
  ...
  const model =
    usable(pick(/^\s*Model\s*[:=]\s*(.+)$/im)) ||
    usable(pick(/Equipment\s*ID\s*[:=]\s*(.+)$/im));
  const equipId =
    usable(pick(/Equipment\s*ID\s*[:=]\s*(.+)$/im)) ||
    usable(pick(/EquipID\s*[:=]\s*(.+)$/im));
  return {
    vendorId: usable(pick(/Vendor\s*ID\s*[:=]\s*(.+)$/im)),
    version: usable(pick(/^\s*Version\s*[:=]\s*(.+)$/im)),
    model,
    equipId,
    sn: usable(pick(/^\s*SN\s*[:=]\s*(.+)$/im)),
    omccVersion: usable(pick(/OMCC\s*Version\s*[:=]\s*(.+)$/im)),
    fields,
    raw: text,
  };
}

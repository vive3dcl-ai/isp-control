/**
 * Helpers WAN del modelo huawei-hgu-veip.
 */
import {
  genieChildIndices,
  genieGet,
...
export function resolveNewWanConnection(
  before: HuaweiWanConnSummary[],
  after: HuaweiWanConnSummary[],
): HuaweiWanConnSummary | null {
  const beforeKeys = new Set(before.map((c) => c.conn));
  const created = after.find((c) => !beforeKeys.has(c.conn));
  if (created) return created;
  return findHuaweiInternetWan(after);
}

export function pickWanConnectionDevice(conns: HuaweiWanConnSummary[]): string {
  if (conns.length) return conns[0].connDevice;
  return `InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1`;
}
